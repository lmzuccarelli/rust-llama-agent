...
REPO="git@github.com:lmzuccarelli/rust-llama-agent.git"
REPO_NAME="rust-llama-agent"
CLEAN=$2
...
clone_build_service() {
  ...
  for host in "${HOSTS[@]}"; do
    ...
    then
      ...
      ssh -i "${PK}" "${USER}@${host}" -tA "cd /home/lzuccarelli/Projects/${REPO_NAME} && rm -rf target/release/*llama* && git pull origin main --rebase && make build"
    fi
  done
}
